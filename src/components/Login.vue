    ...
    <v-form @submit.prevent="login" ref="form">
      <v-card class="mx-auto pa-12 pb-8" elevation="8" max-width="448" rounded="lg">
        ...
        <div class="text-subtitle-1 text-medium-emphasis d-flex align-center justify-space-between">
          Password
          <router-link class="text-caption text-decoration-none text-blue" to="/forgotpassword">
        ...
        <v-text-field
          v-model="password"
          :append-inner-icon="visible ? 'fas fa-eye-slash' : 'fas fa-eye'"
        ...
        <div class="d-flex justify-center align-center mx-10 mb-11">
          <v-btn depressed outlined color="grey" @click="signInWithGoogle" class="d-flex align-center justify-center" style="height: 40px; width: 40px;">
            <v-icon small color="red">fab fa-google</v-icon>
          </v-btn>
        </div>
        <v-card-text class="text-center">
          <router-link class="text-blue text-decoration-none" to="/register2">
            Sign up now <v-icon small>fas fa-arrow-right</v-icon>
          </router-link>
        </v-card-text>
      </v-card>
    </v-form>
